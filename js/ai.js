...
function randomRun(grid, move) {	
	...
	while (true) {
		...
		// If moves are not available
		if (!g.movesAvailable()) 
			break;
		
		// Generate a random move
		var res = g.move(Math.floor(Math.random() * 4));	// move returns score, moved(tile moved or not) and game won or loss 
		//console.log("Number: "+ Object.values(res));
		
		// If the tile is not moved -> continue
		if (!res.moved) 
			continue;
	...
	// grid done.
	// console.log("Score in Random: "+score);
	return {
		score:score, 
		moves:moves
	};
